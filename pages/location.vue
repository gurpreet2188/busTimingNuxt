...
import {Welcome} from "nuxt-app/.nuxt/components";
...
const getData = async (lat: number, lon: number): Promise<{ data: Stop[] | null, error: any | null }> => {
  const query = {lat, lon};
...
<template>
  ...
  >
    ...
    <LoadingPage
        v-else-if="locationError.length > 0 || isStopsError || errorMsg.length> 0"
        :showBarOnly="false"
        :darkTheme="darkTheme"
        :location="isLocationLoading"
        :error="locationError.length > 0 || isStopsError || errorMsg.length> 0"
        :msg="msg || errorMsg"
    />
